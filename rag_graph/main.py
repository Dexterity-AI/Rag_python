...
logger.setLevel(logging.INFO)  # 主程序保持INFO级别

# 禁用第三方库的详细日志
logging.getLogger("neo4j").setLevel(logging.ERROR)  # 禁用Neo4j警告
logging.getLogger("neo4j.notifications").setLevel(logging.ERROR)
logging.getLogger("httpx").setLevel(logging.ERROR)  # 禁用httpx日志
logging.getLogger("httpcore").setLevel(logging.ERROR)
logging.getLogger("openai").setLevel(logging.ERROR)
logging.getLogger("urllib3").setLevel(logging.ERROR)
logging.getLogger("rag_modules").setLevel(logging.WARNING)  # 模块日志只显示警告及以上
...
from rag_modules.intelligent_query_touter import IntelligentQueryRouter, QueryAnalysis
...
class AdvancedGraphRAGSystem:
    ...
    def _show_knowledge_base_stats(self):
        ...
        print(f"   城市/地区: {stats.get('total_cities', 0)}")
        print(f"   景点数量: {stats.get('total_attractions', 0)}")
        print(f"   美食数量: {stats.get('total_foods', 0)}")
        ...
        try:
            ...
            if relevant_docs:
                ...
                for doc in relevant_docs:
                    entity_name = doc.metadata.get('entity_name', doc.metadata.get('name', '未知内容'))
