...
/**
 * 获取项目根目录下的 data/toolbbrowser/fetch 路径
 */
function getDataDir(): string {
  const projectRoot = path.resolve(process.cwd());
  const dataDir = path.join(projectRoot, "data", "toolbbrowser", "fetch");
...
function getDefaultFilename(url: string): string {
  ...
  try {
    ...
    const pathname = urlObj.pathname.replace(/\//g, "_").slice(0, 30);
    return `${hostname}${pathname}_${timestamp}.json`;
  } catch {
    return `fetch_${timestamp}.json`;
  }
}
...
export async function fetchCommand(
  ...
  // 写文件
  const outputPath = options.output || path.join(getDataDir(), getDefaultFilename(url));

  const content = typeof result.body === "object"
    ? JSON.stringify(result.body, null, 2)
    : String(result.body);
  fs.writeFileSync(outputPath, content, "utf-8");
  console.log(`已写入 ${outputPath} (${result.status}, ${content.length} bytes)`);

  // 如果没有指定 output，同时输出内容到控制台
  if (!options.output) {
    console.log("\n--- 响应内容 ---");
